...
export default function List({ res }: Props) {
  ...
  return (
    <LoadingOverlay>
      <LayoutPage name={CURRENCY_PAGE_TITLE}>
          ...
          <Table
            ...
            onDelete={async (item: Currency) => {
              try {
                ...
                if (status) {
                  router.refresh()
                  toast.success(`Delete ${CURRENCY_PAGE_TITLE} Success `)
                } else {
                  toast.error(`Delete ${CURRENCY_PAGE_TITLE} Failed `)
                }
              } catch (error) {
                toast.error(`Delete ${CURRENCY_PAGE_TITLE} Failed `)
              } finally {
