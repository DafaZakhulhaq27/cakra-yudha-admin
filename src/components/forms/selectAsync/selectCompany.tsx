...
export default function SelectCompany(props: Props) {
  const {
    getValues,
    setValue,
    formState: { errors },
  ...
  return (
    <div>
      ...
      >
        Company {props.required && <span className="text-red-500">*</span>}
      </label>
      <AsyncSelect
        ...
        getOptionLabel={_ => _.company_name}
        loadOptions={getData}
        value={
          id
            ? {
        ...
        onChange={v => {
          ...
          setValue(`${props.name}_name` ?? '', v?.company_name)
        }}
        defaultOptions
      />
      {errorMessage && (
